...
server.post("/listboxes/", function (req, res, next) {
  const error = validateBox(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new boxes.
    next();
  }
});
...
function validateBox(box) {
  if (!box.name) return "Name is required.";
  if (!box.weight) return "Weight is required.";
  if (!box.color) return "Colour is required.";
  return "";
}
